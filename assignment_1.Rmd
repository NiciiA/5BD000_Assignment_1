...
# Create the improved graph using ggplot2 (line plot faceted by sex)
p3 <- ggplot(population, aes(x = year, y = n_pop, color = agegroup, group = agegroup)) +
  ...
  # --- Apply X-Axis Fix ---
  scale_x_continuous(
    breaks = year_breaks_pop,             # Use breaks every 5 years
  ...
  theme_minimal() +
  theme(
    legend.position = "bottom", # Move legend for better layout
