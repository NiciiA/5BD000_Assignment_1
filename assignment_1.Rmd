...
cases_agg_total <- aggregate(n ~ agegroup + sex, data = cases, FUN = sum)

# Dodged bars
p1 <- ggplot(cases_agg_total, aes(x = agegroup, y = n, fill = sex)) +
...
cases_yearly <- aggregate(n ~ year + sex, data = cases, FUN = sum)
```
...
p2 <- ggplot(cases_yearly, aes(x = year, y = n, color = sex)) +
...
# Line plot faceted by sex
population$agegroup2 <- ifelse(population$agegroup %in% c("0-4","5-9"), "0–9",
                        ifelse(population$agegroup %in% c("10-14","15-19"), "10–19",
                        ifelse(population$agegroup %in% c("20-24","25-29"), "20–29",
                        ifelse(population$agegroup %in% c("30-34","35-39"), "30–39",
                        ifelse(population$agegroup %in% c("40-44","45-49"), "40–49",
                        ifelse(population$agegroup %in% c("50-54","55-59"), "50–59",
                        ifelse(population$agegroup %in% c("60-64","65-69"), "60–69",
                        ifelse(population$agegroup %in% c("70-74","75-79"), "70–79",
                        ifelse(population$agegroup %in% c("80-84","85-89"), "80–89",
                               NA)))))))))


pop_collapsed <- aggregate(
  n_pop ~ year + sex + agegroup2,
  data = population,
  FUN = sum
)

p32 <- ggplot(pop_collapsed, aes(x = year, y = n_pop, color = agegroup2, group = agegroup2)) +
  geom_line(linewidth = 0.8) +
  facet_wrap(~sex, scales = "free_y", ncol = 1) + # Separate panels for Male and Female
  labs(
    title = "Population Size (Persons at Risk) Over Calendar Years by Age Group and Sex",
...
check_merge <- merged_data[is.na(merged_data$n) | is.na(merged_data$n_pop), ]
...
complete_merged_data <- merged_data[!is.na(merged_data$n) & !is.na(merged_data$n_pop), ]

# Aggregating total cases and total population per year and sex
totals_yearly <- aggregate(
  cbind(total_cases = n, total_population = n_pop) ~ year + sex,
...
merged_data$asir <- merged_data$n / merged_data$n_pop
...
totals_yearly$cir_per_100k <- totals_yearly$cir * 100000
...
```{r}
age_levels <- unique(merged_data$agegroup)
...
target_data$sex <- factor(target_data$sex, levels = c("Female","Male"))
...
model_spline_age <- glm(n ~ ns(year, df = 4) * ns(age_mid, df = 4) + ns(year, df = 4) * sex,
                        offset = log(n_pop),
                        family = poisson(link = "log"),
                        data = df_q11)
