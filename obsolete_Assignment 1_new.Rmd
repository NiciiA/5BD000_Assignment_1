...
```{r}
# ns(year, df = 4) * sex + ns(age_mid, df = 4) * sex
model_spline_age <- glm(n ~ ns(year, df = 4) * sex + ns(age_mid, df = 4) * sex + offset(log(n_pop)),
                        family = poisson(link = "log"),
                        data = df_q11)
...
ggplot(df_comparison, aes(x = year, y = rate, color = sex, linetype = method)) +
  ...
  scale_color_manual(values = c("Male" = "#1f77b4", "Female" = "#d62728")) +
  scale_linewidth_manual(values = c("Observed (Direct)" = 0.6, "Model-Based (Predicted)" = 1.2)) +
